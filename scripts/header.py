    ...
    with open(file, 'w') as f:
        ...
        for (section_name, section_natives) in natives_json.items():
            ...
            for (native_hash, native_info) in section_natives.items():
                ...
                comment = f'// b' + native_info['first_build']

                if show_comments:
                    if native_info['comment']:
                        f.write('\n'.join([ f'{indent}// {line}' for line in native_info['comment'].strip().split('\n') ]) + '\n')

                f.write(indent + f'static inline {return_type} {name}({params_full_string}) {{ return _i<{return_type}>({params_name_string}); }} {comment}\n')

            f.write(f'}}\n')

save_natives_json(load_natives_json('natives.json'), 'natives.h', enum_hashes = False, show_comments = False)
